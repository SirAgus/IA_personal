...
export function MainSidebar({
    ...
    const isLocalMode = mode === 'v2';

    const loadData = async () => {
        try {
            if (isLocalMode) {
                const [localThreads, localAgents] = await Promise.all([
                    localDb.listThreads(),
                    localDb.listAgents()
                ]);
    ...
    useEffect(() => {
        loadData();
    }, [currentThreadId, selectedAgentId, token, refreshTrigger, mode]);
    ...
    const handleSubmitAgent = async (e: React.FormEvent) => {
        ...
        if (isLocalMode) {
            if (editAgentId) {
                await localDb.updateAgent(editAgentId, payload);
            } else {
                await localDb.createAgent(payload);
            }
        } else {
            if (!token) return;
    ...
    const saveThreadEdit = async () => {
        ...
        if (isLocalMode) {
            await localDb.updateThread(editingThreadId, { title: editThreadTitle, agent_id: editThreadAgentId });
    ...
    const displayName = isLocalMode ? 'Local' : user ? user.username : 'Invitado';
    const statusLabel = isLocalMode ? 'SQLite local' : token ? 'En línea' : 'Sin conexión';
    const statusActive = isLocalMode || Boolean(token);
        ...
        <>
            ...
            `} style={{ paddingTop: 'env(safe-area-inset-top)', paddingBottom: 'env(safe-area-inset-bottom)' }}>
                ...
                <div className="p-5 border-b border-black/5 dark:border-white/5 flex items-center justify-between">
                    <h1 className="text-xl font-black text-black dark:text-white font-mono tracking-tighter">Athenas AI</h1>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-4">
                    <div className="space-y-3">
                        <div className="bg-black/5 dark:bg-white/5 rounded-2xl p-1 flex gap-1">
                            <button
                                type="button"
                                onClick={() => onModeChange('v1')}
                                className={`flex-1 text-[10px] font-bold uppercase tracking-widest py-2 rounded-xl transition-all ${mode === 'v1'
                            ...
                            <button
                                ...
                                className={`flex-1 text-[10px] font-bold uppercase tracking-widest py-2 rounded-xl transition-all ${mode === 'v2'
                ...
                <div className="p-4 bg-black/5 dark:bg-black/20 border-t border-black/5 dark:border-white/5 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold text-gray-900 dark:text-gray-200 truncate">{displayName}</p>
                        <p className={`text-[10px] font-bold flex items-center gap-1.5 mt-0.5 ${statusActive ? 'text-green-600 dark:text-green-500' : 'text-gray-400'}`}>
                           <span className={`w-1.5 h-1.5 rounded-full ${statusActive ? 'bg-green-500 animate-pulse' : 'bg-gray-400'}`}></span>
                           {statusLabel}
                        </p>
                    </div>
                </div>
